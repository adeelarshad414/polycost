...
const requiredFiles = [
  ...
  'docs/architecture/phase-2-diagram-llm-drift-alert-evidence.md',
...
if (!packageJson.scripts?.['pricing:catalog:snapshot:capture:run:plan']) {
  failures.push('package.json is missing pricing:catalog:snapshot:capture:run:plan');
}
if (!packageJson.scripts?.['pricing:catalog:snapshot:capture:run:smoke']) {
  failures.push('package.json is missing pricing:catalog:snapshot:capture:run:smoke');
}
...
if (!packageJson.scripts?.['diagram:llm-corpus:capture']) {
  failures.push('package.json is missing diagram:llm-corpus:capture');
}
...
await assertFileContains('README.md', [
  ...
  ['diagram LLM drift alert checker command', 'npm run diagram:llm-corpus:drift:alert:check'],
  ...
  [
    'diagram LLM drift alert architecture link',
    'docs/architecture/phase-2-diagram-llm-drift-alert-evidence.md',
  ],
  [
...
await assertFileContains('docs/HOW-TO-USE.md', [
  ...
  ['diagram LLM drift alert checker', 'npm run diagram:llm-corpus:drift:alert:check'],
...
await assertFileContains(
  'docs/operations/evidence/diagram-llm-corpus-capture/diagram-llm-corpus-capture.example.json',
  [
    ['diagram LLM capture example schema', 'polycost-diagram-llm-corpus-evidence-capture/v1'],
    ['sample-only evidence level', 'example-schema'],
...
await assertFileContains('docs/architecture/phase-2-diagram-llm-corpus-evidence-capture.md', [
  ['diagram LLM capture title', 'Phase 2 Diagram LLM Corpus Evidence Capture'],
  ['diagram LLM capture smoke command', 'npm run diagram:llm-corpus:capture:smoke'],
