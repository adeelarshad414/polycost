...
- Confirm local diagram-classifier drift alert sender/receiver proof passes with
  `npm run diagram:llm-corpus:drift:alert:smoke` before demoing alert handoff.
...
- Confirm provider catalog snapshot comparison proof passes with
  ...
  `npm run pricing:catalog:snapshot:capture:preflight` for advisory readiness,
  run
...
- Run `npm run pricing:catalog:snapshot:capture:preflight`.
