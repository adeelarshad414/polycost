...
| # | Finding | Evidence |
|---|---|---|
| **H1** | **Managed-DB data storage silently dropped (default path).** A Postgres DB with `sizeGb:500` is billed instance-hours only; ~$57–115/mo of DB storage vanishes. No storage catalog row exists in DB fixtures. | `base-cloud-provider.adapter.ts:165-182`; `mock-pricing-fixtures.ts:592-618` |
...
- **N1 (NEW, HIGH) — Enabling live mode did not yield live pricing.** With `USE_MOCK_PROVIDERS=false`, the API booted, GCP failed on the missing token, and AWS/Azure persisted **zero** live catalog rows (`pricing_catalog` remained 100% `fixture://` mock + `local_seed`, newest row predating the live boot). Per-provider status still read `success`/`fresh`. The served numbers stayed mock. Only the B2 provenance fix exposes this (`usesNonLivePricing: true`). Root cause (live AWS/Azure boot ETL not persisting) needs a dedicated fix before any live claim.
- **N2 (NEW, HIGH) — Azure compute under-selection.** The harness flagged Azure compute-only at $60.74 vs a $140.16 reference (−56.7%): the engine selects an under-spec cheaper row (2 vCPU/**4** GB seed at $0.0416) for a 2 vCPU/**8** GB requirement, while AWS correctly picks the 8 GB SKU. Memory is not enforced in Azure compute selection. `apps/api/src/adapters/common/base-cloud-provider.adapter.ts` selection path.
